# XM Cloud Next.js Tailwind CSS Starter Kit - Disconnected offline mode
...
If you need to change the working directory, you can modify the path in the provided scripts to point to your desired location. Ensure that all paths are updated consistently to avoid any issues when running the containers.
...
You first need to initialize the repository, which will configure how the different application elements will run. This will configure the different environment variables required for the Containers to build and run. You can do this by running the `./scripts/init.ps1` script in a terminal with elevated privilidges:
...
After you have initialised the repository you can use the `./scripts/up.ps1` script to build and run the containers:
...
Once you have finished you can use the `./scripts/down.ps1` script to stop the containers:
